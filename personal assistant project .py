...
import smtplib  # required to work with q#from playsound import playsound # required to play music
...
def speak(audio):  # function for assistant to speak
    engine.say(audio)
    engine.runAndWait()  # without this command, the assistant won't be a
...
def takecommand():  # function to take an audio input from the user
    ...
    try:  # error handling
        print('Recognizing...')
        query = r.recognize(audio)  # using google for voice recognition
...
if __name__ == '__main__':  # execution control
    ...
    while True:
        ...
        if 'wikipedia' in query:
            ...
            query = query.replace('https://www.wikipedia.org/', '')
            ...
            speak('I was created by my boss Yash sharma')
